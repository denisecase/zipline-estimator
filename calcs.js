...
export function calculateZiplineGeometry(params) {
    ...
    const relativeStartGroundElevationFt = relativeEndGroundElevationFt + slopeDeltaFeet;

    // --- Step 2: Establish Initial Relative Anchor Elevations ---
    const relativeEndAnchorElevationFt = relativeEndGroundElevationFt + initialEndAnchorHeightFeet;
    const relativeStartAnchorElevationFt = relativeEndAnchorElevationFt + cableDropFeet;

    // --- Step 3: Calculate Initial Relative Cable Midpoint Elevation ---
    const midlineAnchorElevationFt = (relativeStartAnchorElevationFt + relativeEndAnchorElevationFt) / 2;
    ...
    const relativeBottomClearanceElevationFt = relativeMidCableElevationFt - seatDropFeet - clearanceFeet;
    ...
    if (relativeBottomClearanceElevationFt < relativeEndGroundElevationFt) {
        verticalOffsetNeededFt = relativeEndGroundElevationFt - relativeBottomClearanceElevationFt;
    }
    ...
    const finalStartGroundElevationFt = relativeStartGroundElevationFt + verticalOffsetNeededFt;
    const finalEndGroundElevationFt = relativeEndGroundElevationFt + verticalOffsetNeededFt;
    const finalStartAnchorElevationFt = relativeStartAnchorElevationFt + verticalOffsetNeededFt;
    const finalEndAnchorElevationFt = relativeEndAnchorElevationFt + verticalOffsetNeededFt;
    const finalMidCableElevationFt = relativeMidCableElevationFt + verticalOffsetNeededFt;
    const finalBottomClearanceElevationFt = relativeBottomClearanceElevationFt + verticalOffsetNeededFt;

    // --- Step 7: Calculate Horizontal Positions (in feet) ---
    const startXFt = 0;
    const endXFt = runFeet;
    const midXFt = runFeet / 2;

    // --- Step 8: Calculate Label Values (in feet) ---
    const startAnchorAboveStartGroundFeet = finalStartAnchorElevationFt - finalStartGroundElevationFt;
    const startAnchorAboveEndGroundFeet = finalStartAnchorElevationFt - finalEndGroundElevationFt;
    const endAnchorAboveEndGroundFeet = finalEndAnchorElevationFt - finalEndGroundElevationFt;
